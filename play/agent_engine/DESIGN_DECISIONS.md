...
本文档记录 `play/agent_engine/` 的系统设计决策，按时间顺序积累。每节包含背景、候选方案、选择依据、行业光谱位置，以及按统一工程维度的评估。维度定义见附录。

> 历史包名：本子项目原名 `play/multiagent/`，2026-04 重构为 `play/agent_engine/`（双层架构 v2，详见仓库根 plan）。下文中"multiagent"在历史叙述中保留原文不变；当前包名为 `agent_engine`。
