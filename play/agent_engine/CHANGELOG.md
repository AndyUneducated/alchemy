...
4. **装配点集中**：`Scenario.assemble()` 产出运行时对象图，`Engine.invoke()` 负责单次运行的编排与落盘；`cli.py` 只做 argparse → 上述 API。讨论内核不依赖 CLI 或脚本入口（详见 §10）
...
- 与文首指导原则「显式优于隐式」「装配点集中」对齐：`assemble` 与 `invoke` 分工比「一个上帝脚本」更易陈述不变量
