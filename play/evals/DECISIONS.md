...
## 8. Phase 8 实现：族 1 后半 IAA + LLM-as-annotator 交叉（kappa paradox 章节）
...
| 模块 | 内容 |
|---|---|
| 新 task `iaa_nominal` | 30 条 highly imbalanced (27 ham + 3 spam, ~90/10) binary classification + 4 份 stub predictions（perfect / constant_majority / noisy_diverging / garbage）+ 3 raters/sample；aggregation 输出 15 stat（9 classification + 3 agreement 2-rater + 2 multi-rater + 1 `_confusion_matrix` 诊断辅助） |
| 新 task `iaa_ordinal` | 25 条 1-5 likert (5 each) + 4 份 stub（perfect / off_by_one / random / garbage）+ 3 raters/sample；aggregation 12 stat（1 exact + 3 agreement nominal/linear/quadratic + 3 corr + 1 ccc + 4 multi-rater） |
| 新 metric 模块 `metrics/agreement.py` | 4 个手算（`scott_pi` / `gwet_ac1` / `lins_ccc` / `icc_1_1`）+ 1 个共享 helper `build_rater_matrix(srs, include_gold=True)`，~80 行；scope 收紧（见下） |
| 库直调下放 task aggregation | sklearn `cohen_kappa_score(weights=...)` / scipy.stats `pearsonr|spearmanr|kendalltau` / statsmodels `fleiss_kappa` + `aggregate_raters` / `krippendorff.alpha(level_of_measurement=...)` 全部 task 内 import + 调用 |
| CLI dispatch 0 改动 | IAA task 不接 judge / vdb，自然走 `_build_task_with_optional_deps` 既有 fall-through 分支（与 sentiment_clf / mt 同形）；非法 flag 组合仍 SystemExit（既有兜底机制） |
| 依赖增量 | `requirements.txt` 加 `statsmodels>=0.14` + `krippendorff>=0.6`；显式不引 `irrCAC` / `pingouin` / `audtorch` |
| 测试增量 | +52：`test_metrics_agreement.py` 29 + `test_iaa_nominal_score.py` 10 + `test_iaa_ordinal_score.py` 9 + `test_cli_spec.py` IAA 段 4；全量 299 → 351 |

### Implementation

| 侧面 | 做法 |
|---|---|
| `output_type='none'` | 与 phase 4 `rag_retrieval` / phase 5 `agent_traj` 同型——runner 跳 `lm.generate_until` 直接给占位 `Response(doc_id=d.id)`；score 路径 `task.load_prediction(doc, row)` 把 `raters` 注入 `doc.metadata`；`process_results` 转写到 `SampleResult.artifacts["raters"]` |
| 数据契约 0 新概念 | predictions JSONL 行 schema `{id, prediction, raters: list[str\|int]}`；与 phase 4 立的 `task.load_prediction` hook + path B+C「LM-side 数据装 Response，pipeline 产物住 doc.metadata」自然吻合；不动 `api.py` 任一 dataclass / 不动 `Task` ABC（仅 override `load_prediction` + `process_results`）/ 不动 `runner.py`（IAA task 走 `output_type='none'` 既有路径）/ 不引新 CLI flag |
| 4 个手算函数 | `scott_pi` (~10 行 Pe 用合并边际 ∑p̄_c²) / `gwet_ac1` (~15 行 Pe = (1/(K−1))·∑q_c(1−q_c)) / `lins_ccc` (~5 行 2·cov/(σ_X²+σ_Y²+(μ_X−μ_Y)²)) / `icc_1_1` (~12 行 one-way random ANOVA decomposition (BMS−WMS)/(BMS+(K−1)·WMS))；TDD textbook example 锁数值（如 imbalanced 90/10 → cohens_kappa=0 ∧ gwet_ac1≈0.89；off_by_one → cohens_kappa=−0.25 ∧ weighted_quad=0.71） |
| `build_rater_matrix` 唯一共享 helper | N samples × K raters → N×(K\|K+1) `list[list[Hashable]]`；`include_gold=True` 拼 target 作为 column 0；fail-loud 缺 raters / uneven raters；fleiss / krippendorff / icc 三者统一入口 |
| 教学叙事数值锚 | `iaa_nominal.constant_majority`：accuracy=0.9 ∧ cohens_kappa=0 ∧ gwet_ac1≈0.89（kappa paradox + 解药）/ `iaa_nominal.noisy_diverging`：cohens_kappa=0.26 mid 但 fleiss_kappa<0（多 rater 拉平）/ `iaa_ordinal.off_by_one`：accuracy=0 ∧ cohens_kappa=-0.25 但 weighted_quad=0.71 ∧ pearson=0.83 ∧ ccc=0.71（ordinal 救场）/ `iaa_ordinal.garbage`：pred=6−gold inverse → weighted_quad=−1 ∧ pearson=−1 ∧ ccc=−1 但 cohens_kappa=0（paradox 反向复刻）|

### Options considered

**① 双 task vs 单 task**：

- **双 task (选)**：`iaa_nominal` (kappa paradox 主舞台) + `iaa_ordinal` (ordinal-aware vs nominal κ 救场叙事)；两个教学故事都干净，aggregation 数量各自合理（15 + 12 stat）
- 单 task：把 nominal + ordinal 全塞一个 task → aggregation 膨胀到 25+ stat 且每个 prediction 只演一个故事；混合数据集（部分 binary 部分 ordinal）让 process_results 逻辑分支化、test fixture 体量翻倍；教学叙事互相稀释（kappa paradox 与 ordinal 救场是两个独立维度）

**② Multi-rater 数据源 — inline JSONL raters vs self_consistency rater 派生**：

- **inline JSONL `raters` 字段 (选)**：predictions JSONL 行 schema `{id, prediction, raters: [...]}`；与 phase 4 立的 `task.load_prediction` hook + path B+C 自然吻合；零新概念
- self_consistency 派生：用 `judge_lm` 多次采样产生 N 个 rater label（族 1 ↔ 族 3 真正交叉的"判 LM 当 annotator"形态）；优势是 run 路径完整教学，但需新增 `--judge-n-samples` flag + 与 judge_pointwise 区分（pointwise 输出分数，annotator 输出标签）+ 仅适用 run 路径（score 还是要走 inline）。两条路径形态分裂，且本期 score 主路径已能完成所有数值教学。**Self-consistency annotator 派 deferred 至 phase 8.5+**

**③ ICC(1,1) only vs 全 ICC(2,1)/(3,1)**：

- **ICC(1,1) one-way random only (选)**：单 ANOVA decomposition，~12 行手算；workshop 体量（25-30 sample / 3 raters）下数值已较稳
- 全 ICC(2,1) / ICC(3,1)：需要二阶 between-rater sum of squares 分解，工程量翻倍；workshop 体量未必能给出稳定数值（小 N 下 ICC(2,1) variance 大）。**ICC(2,1) / ICC(3,1) deferred 至 phase 8.5+**

**④ statsmodels + krippendorff vs irrCAC vs 全手算**：

- **statsmodels + krippendorff + 4 个手算 (选)**：statsmodels 已是 phase 8 之前依赖图近邻（scikit-learn 同生态），fleiss_kappa + aggregate_raters 是行业标准实现；krippendorff 单一 API 覆盖 nominal/ordinal/interval 三 level；4 个简单统计量手算（公式 5-15 行）避 `irrCAC` (gwet 唯一用法) / `pingouin` (ICC 唯一用法) / `audtorch` (ccc 唯一用法 + torch 全套依赖) 三个本可用的库
- irrCAC 全替换：irrCAC 提供 gwet_ac1 + krippendorff 等全套，但 gwet 公式只 15 行手算就够，且 irrCAC API 不稳定
- 全手算（含 fleiss / krippendorff）：fleiss / krippendorff 公式真复杂（krippendorff 全 level 实现要数百行），自造无收益
- 选项 1 是 minimal-deps 中庸方案：合理引 2 个成熟库 + 4 个简单手算

**⑤ run 路径完整教学 deferred 同源 phase 5**：

- **score 主路径 + run 路径 deferred (选)**：IAA task `output_type='none'`，runner 给占位 `Response`；run 路径 `process_results` 看不到 raters → aggregated 仅给 sanity 0；score 主路径已覆盖全部教学叙事
- run 路径完整：需要在 `--judge-model` 之外新增 `--judge-n-samples` flag + judge LM 当 annotator 的 prompt 模板设计 + score / run 输出 parity 重新焊死。本期 ROI 不够。**完整 run 路径教学 deferred 至 phase 8.5+**（与 phase 5 agent_traj `--replay-envelope` 同源让步）

**⑥ `metrics/agreement.py` scope —— 收紧 vs 包含库 wrap**：

- **收紧 scope (选)**：仅 4 个手算 + 1 个真共享 helper，~80 行；库直调全部下放 task aggregation
- 包含库 wrap（如 `fleiss_kappa(srs)` / `krippendorff_alpha(srs, level)` 等 closure factory 形态）：3-5 行 wrap 的"协议转接"价值低（statsmodels.fleiss_kappa 接口直接吃 matrix，无需 SampleResult → matrix 协议转接 helper 之外的额外 closure）；与 sentiment_clf 直调 sklearn / mt 直调 sacrebleu 的项目体例不一致；模块沦为 import 中转站。
- 唯一例外（与本决策不冲突）是 phase 4 `metrics/retrieval.py`：那里 wrap 是「协议转接（closure factory）+ 输入构造非平凡（30 行 `_build_qrels_run` SampleResult → Qrels/Run dict）+ 5 指标共用 helper」三联立信号；本模块 statsmodels / krippendorff 接口直接吃 matrix，1-3 行 wrap 没有协议转接价值。
- **下放 task aggregation 让 task 形成"自包含 metric 调用单元"**：sentiment_clf / mt / iaa_* 同形——读 README 看任一 task 即可理解其全部 metric 来源（sklearn / sacrebleu / statsmodels），不需跳到 metrics/ 模块再翻 wrap

### Decision

- **双 task** 上线 `iaa_nominal` (kappa paradox 主舞台) + `iaa_ordinal` (ordinal-aware vs nominal 救场)；两个教学叙事独立可读
- **`metrics/agreement.py` 收紧 scope**：仅装手算 + 真共享 helper（4 函数 + 1 helper, ~80 行）；库直调全部下放 task aggregation；与 sentiment_clf / mt 体例完全一致
- **inline JSONL raters** 作 multi-rater 数据契约；与 phase 4 立的 `task.load_prediction` hook + path B+C 一致；零新概念
- **ICC(1,1) only / run 路径完整教学 / `--judge-n-samples` annotator 派**全部 **deferred 至 phase 8.5+**

### Risks logged（与 §6.1 / §7.1.R1 同体例）

| ID | 风险 | 触发条件 | 后果 | 当前 mitigation |
|---|---|---|---|---|
| §8.R1 | krippendorff alpha 在 N<10 时方差大（workshop 体量约束）| 用户用 `--limit 5` 等小 sample 跑 IAA task 时 alpha 数值不稳 | drill-down 用户看到 alpha 摆动易误读为"指标本身有问题" | README phase 8 段标注 25-30 sample 数据集体量是 **教学下限**；真实评测请用 N≥50 的数据集；与 phase 7 §7.1.R1（judge LM variance）同精神（指标 / LM 内禀局限非工程问题） |
| §8.R2 | `_confusion_matrix` 嵌套 dict 在 cross-run JSON_EXTRACT / dashboard 路径上下钻深 | 未来加 grafana / SQLite read model 时 schema-on-write 不再是 leaf scalar | dashboard 渲染层需要按 path 拆嵌套 dict | 用 `_` 前缀显式标"诊断辅助 / 非 first-class 指标"，higher_is_better 字典不出现该键；下游消费者（如 W&B）可按 prefix 跳过；接 dashboard 时若需 first-class 化再 promote 为顶层 9 个 confusion-derived scalar（`tp_spam` / `fp_spam` / ... 共 4 cell × 2 class = 8 - 重复 = 4 实际 ）|
| §8.R3 | ICC(2,1) / ICC(3,1) deferred 但用户期望"完整 ICC 套件" | 用户调研 / 实际评估 task 需要区分 raters as random sample (ICC 1,1) vs raters fixed (ICC 2,1) vs raters with absolute agreement (ICC 3,1) | 教学叙事 deferred → 用户绕到 `pingouin` 直调 | DECISIONS §8 显式登记 deferred + 在 README C.1 表标注 ICC(1,1) 已落地 / ICC(2,1)(3,1) deferred；触发条件（实际 user feedback / 与 fairness 维度联动）满足时 phase 8.5+ 单独成 ADR 实现 |
| §8.R4 | 退化输入下库直调 raise / emit NaN — `realized → mitigated` | ① `--limit` 缩到只剩单类 → sklearn `precision_score(pos_label='spam')` raise + `krippendorff.alpha` raise；② run path `output_type='none'` 给占位 `Response(text=None)` → pred = `""` 让 sklearn binary scorers 看到 multiclass `{ham, spam, ""}` raise / `cohen_kappa_score` Pe=1 NaN / `pearsonr` 等常数输入 NaN；③ 二次审计补刀：scipy `pearsonr/spearmanr/kendalltau` 在 N<2 raise `ValueError: x and y must have length at least 2`；④ `iaa_ordinal` 的 `build_rater_matrix` 把 `sr.target` (str) + `raters` (int) 混入同一矩阵，dtype-aware unique 检测必须在 `np.asarray(dtype=int)` 转换之后做（之前在 raw matrix 上做检测假性给 unique=2，krippendorff 实际看到 single-domain 仍 raise）；⑤ storage 层 `json.dumps` 默认 `allow_nan=True` 写出 `NaN` / `Infinity` 字面量（非合法 JSON），任意未来 task 漏算就会污染 cross-run 索引；NaN 还污染 `EvalResult.aggregated` JSON 序列化 | CLI 直接 `SystemExit` / 跨 run JSON_EXTRACT / jq / 浏览器 / DB / 仪表盘消费失败 | 三道防线：① **task aggregation 内 task-local closure helper**：`_pos_label_present(yt, yp)` 短路 sklearn binary scorers；`<2 unique value` 短路 `krippendorff.alpha`（iaa_ordinal 的 unique 检测在 `np.asarray(dtype=int)` 之后做）；`len(srs) < 2` 短路 scipy 相关；`_nan_to_zero(x)` (用 `x != x` 检测 NaN) 包所有可能 NaN 的路径（cohens_kappa / weighted_kappa_* / fleiss_kappa (两 task 都包) / pearsonr / spearmanr / kendalltau / krippendorff alpha / icc_1_1）；② **storage 层 strict-JSON 兜底**：`storage.save` 三处 `json.dumps(..., allow_nan=False)` fail-loud 拒绝 NaN/Inf 写盘 — 即使任意未来 task 漏算第一道防线，写盘时立即 `ValueError` 而非静默写出毒文件后下游才崩；③ **测试 `test_iaa_engineering_robustness.py` (19 条)**：`_UnusedLM` (LM 调用即 AssertionError) 锁 `output_type='none'` 真不调 + `_assert_aggregated_is_finite_json` helper (`math.isfinite` 全 leaf + `json.dumps(allow_nan=False)` round-trip) 锁 NaN 不泄露 + `--limit 0/1/2` 极小 slice 不 raise / 不 NaN + `parse_constant=raise ValueError` 模拟 jq / 非 Python parser 检 storage 输出严格合法 JSON + 合成 NaN/Inf EvalResult 验 `storage.save` 真 fail-loud；2026-05-07 follow-up 双轮里程碑落地（首轮发现 4 类 raise/NaN 表面 bug；二轮 audit 在补测中又抓到 3 处更深的 bug：scipy 长度<2 raise / dtype 混合 unique 检测假性 / storage 层 NaN 写盘） |

### Supersession 链

无（本 ADR 是 phase 8 首次落地，不 supersede 任何前置 ADR；与 §1（架构原则）/ §4（path B+C 数据契约）/ §5（output_type='none' run 路径让步）/ §7.A 单一 call class（被测物 vs 评估工具）兼容继承）。

## 8.1 Phase 8 audit follow-up wave 3：OOV / invalid prediction 数据契约 + warnings 静音
...
| 模块 | 改动 |
|---|---|
| [`tasks/iaa_ordinal.py`](tasks/iaa_ordinal.py) `process_results` | `pred_int=0 fallback` → `pred_int=None + _pred_invalid: True` artifact；保留 raw `pred_str` 在 `prediction` 字段（drill-down 价值 > `'None'` 字符串）|
| [`tasks/iaa_ordinal.py`](tasks/iaa_ordinal.py) `aggregation` | `_y_int(srs)` → `_y_int_valid(srs)` filter `_pred_invalid`；6 个 OOV 敏感 metric (`cohens_kappa` / `weighted_kappa_linear` / `weighted_kappa_quadratic` / `pearson_r` / `spearman_rho` / `kendall_tau` / `lins_ccc`) 切 valid subset；`accuracy` 走 `mean(s.metrics["acc"])` 不再 `accuracy_score`（避 None sentinel 进 sklearn 路径）；3 个相关性 metric 加 `_is_constant(yt) or _is_constant(yp)` 短路 |
| [`tasks/iaa_nominal.py`](tasks/iaa_nominal.py) `process_results` | 新写 `_pred_invalid: pred not in LABELS` artifact；`prediction` 仍存 raw（保留诊断价值）|
| [`tasks/iaa_nominal.py`](tasks/iaa_nominal.py) `aggregation` | `_y(srs)` 全部 sample（`accuracy` / `_confusion_matrix` 用）vs `_y_valid(srs)` 切片（OOV 敏感 metric 用：`balanced_accuracy` / `mcc` / `f1_micro` / `f1_macro` / `f_beta_2` / `precision_spam` / `recall_spam` / `f1_spam` / `cohens_kappa` / `scott_pi` / `gwet_ac1`）；`balanced_accuracy` / `mcc` / `cohens_kappa` / `fleiss_kappa` 各包 `warnings.catch_warnings()` 局部静音 sklearn / statsmodels 退化路径的 `RuntimeWarning` / `UserWarning` |
| 测试增量 | `test_iaa_engineering_robustness.py` +8 条 audit follow-up wave 锁：①-③ P0 mixed invalid + all-invalid + real predictions 不变；④ P1a no OOV warning；⑤ P1a no sklearn RuntimeWarning；⑥ P1b no ConstantInputWarning；⑦ P2 no single-label UserWarning；19 → 27 |
| 全量回归 | 367 → 375 测试通过（+8 audit lock），warnings **43 → 1**（仅留 `unknown pricing model 'fake:judge'` phase 6 fail-loud 预期 warn）|

### Implementation

| 侧面 | 做法 | 哲学 |
|---|---|---|
| 数据契约 | `_pred_invalid: bool` artifact + raw `prediction` 字段保留 | 与 phase 4 path B+C「非标量产物住 artifacts」+ phase 7 §7.D「nested 派 / scalar 顶层平铺」体例一致；invalid 状态走 artifacts 不污染 `metrics` scalar 契约 |
| `accuracy` 用全部 sample | OOV pred 自然不等于 target → 0 贡献，不缩 N；走 `mean(s.metrics["acc"])` 而非 sklearn `accuracy_score` 避 None sentinel 路径 | N 在分母固定 = 教学叙事数值稳定（`constant_majority.acc=0.9` 不会因为 invalid 比例变化而漂移）|
| OOV 敏感 metric 用 valid subset | 切片后 sklearn 看到的 yp 严格 ⊆ labels；`labels=[1..5]` 不再静默丢弃；空 subset → 0 短路 | sklearn `cohen_kappa_score(..., labels=[...])` 对 yp ∉ labels 静默丢弃是文档行为（不是 bug）；切片让"哪些样本进 metric"对消费者透明 |
| multi-rater metric 用全部 sample | `build_rater_matrix(srs, include_gold=True)` 装 `[target, *raters]`，**不含 pred** → 不被 OOV 污染 | matrix 列结构与 OOV pred 正交，无需 filter |
| `warnings.catch_warnings()` 局部静音 | `_balanced_accuracy` / `_mcc` 包 `UserWarning`；`_cohens_kappa` / `_fleiss_kappa` / `_weighted_kappa_*` 包 `RuntimeWarning` | 退化路径 stderr 干净；外层 `_nan_to_zero` 仍兜数值；与"显式让步 > 不准估算"哲学一致——warning 是渲染层噪音，数值层已在第一道防线兜住 |
| `_is_constant(xs) = len(set(xs)) < 2` 短路 | `pearson_r` / `spearman_rho` / `kendall_tau` 入口判 yt / yp 是否常数，常数则返 0；与 `_krippendorff_alpha_*` 的 `len(np.unique(rd)) < 2` 风格对齐 | scipy `ConstantInputWarning` 是统计上的合理警告（常数 input 相关性确实未定义），但本 task 教学场景下 0 是约定 sanity 值，预先短路比让 scipy warn 更干净 |

### Options considered

**① P0 invalid sentinel 设计**：

- **方案 A `_pred_invalid: bool` artifact + aggregation valid subset (选)**：契约清晰；`accuracy` 用全部 sample / OOV 敏感 metric 用切片，N 不漂移；与 phase 4 path B+C 体例一致
- 方案 B `pred_int=-1` sentinel：仍可能与 sklearn `labels=[1..5]` 的"label 集外静默丢弃"行为冲撞；要求所有 metric 调用方都额外 filter `-1`，重复劳动
- 方案 C `process_results` raise：fail-loud，但教学场景 LM 输出非整数是合理可预期现实；raise 会让单条 sample 拖垮整个 run（与 phase 7 立的「LM 内禀局限非工程问题」精神不符）
- 方案 D fallback to LIKERT 中位 (3)：metric 数值被人为掺杂"假数据"，污染教学叙事

**② OOV 敏感 metric 切片 vs 全 metric 切片**：

- **OOV 敏感 metric 切片（选）**：`accuracy` / `_confusion_matrix` / multi-rater metric 走全部 sample 保 N 稳定；其余切 valid subset
- 全 metric 切片：accuracy 也 filter 后 N 缩水，教学叙事数值漂移（如 `constant_majority` 中如果 N 缩到 27，原本 27/30=0.9 会变成 27/27=1.0）

**③ warnings 处理 — 静音 vs 上游清洁输入**：

- **`warnings.catch_warnings()` 局部静音 (选)**：影响范围限于退化路径（单类切片 / Pe=1）；外层 `_nan_to_zero` 仍兜数值
- 上游清洁输入：把单类切片归一化为多类（如插入虚拟 sample），破坏教学叙事的"limit=5 全 ham 单类切片"演示价值

**④ `accuracy` 走 sklearn vs 直接平均**：

- **直接平均 `mean(s.metrics["acc"])` (选)**：`_pred_invalid → metrics["acc"]=0.0` 在 process_results 内已锁；不需要再绕一次 sklearn；避 None sentinel 进 sklearn 路径
- 走 `accuracy_score(yt, yp)`：要求 yp 中 None → str("None") 或类似 placeholder；与 sklearn `_check_targets` 的"y_pred contains classes not in y_true"路径冲突

### Decision

- **P0 修复**采用 `_pred_invalid` artifact + valid subset 切片；`accuracy` / `_confusion_matrix` / multi-rater metric 走全部 sample
- **P1a/P1b/P2 修复**用 valid subset 切片 + 局部 `catch_warnings()`，外层 `_nan_to_zero` 仍兜数值
- 与 §8.R4 wave 1+2 的 mitigation 累加（同源 phase 6/7 audit follow-up wave 体例）；§8.R4 status 升级为 `realized → mitigated (wave 1) → deepened (wave 2: scipy len<2 / dtype unique / storage strict-JSON) → contract-tightened (wave 3: invalid sentinel + warning silence)`

### Risks / 显式让步

| 项 | 内容 |
|---|---|
| `_pred_invalid` 不进 `aggregated` 顶层 | 当前 wave 不在顶层暴露 invalid_count / invalid_rate；下游消费者要从 sample.artifacts drill-down 数。如未来加 dashboard 需 cross-run 比较 invalid rate，再 promote 为 `aggregated["_invalid_rate"]` |
| `prediction` 字段对 invalid 行存 raw `pred_str`（不归一化）| drill-down 看 LM 真实输出 > `'None'` 字符串；但 cross-run 比较 prediction 字段时同条 doc 在不同 run 间 prediction 可能差异大（合法的——原本就是 LM 输出形态 audit）|
| iaa_ordinal `raters` 仍走 `int(r) except 0` fallback | rater_ints 进 multi-rater matrix（不含 pred），fallback 影响仅限 fleiss/krippendorff/icc_1_1 的统计平滑，远小于 P0 主问题；与 wave 3 minimal-diff 范围一致 |

### Supersession 链

- §8.R4 status 在本 ADR 升级（不删旧文，append `wave 3 contract-tightened`）；wave 1+2 mitigation 全部保留有效，wave 3 在数据契约层面进一步收紧

## 8.2 Phase 8 audit follow-up wave 4：judge closure 解析失败 → None propagation（E1 修复）
...
Phase 1-8 全量真实 LM 测试在 [`tasks/agent_traj.py`](tasks/agent_traj.py) `score+judge garbage.jsonl` 路径触发 P0 工程缺口：judge LM 输出"基于这些信息，没有结构化的计划被执行..."这种解释性文本但不含数字时，[`metrics/judge_core.py::parse_pointwise_score`](metrics/judge_core.py) line 114 抛 `ValueError`，[`g_eval._score`](metrics/judge_core.py) line 292（pre-fix）+ [`judge_pointwise._score`](metrics/judge_core.py) line 179（pre-fix）两个 call site 不捕获 → 整个 run 中断；已耗 ~140s + 同 batch 中已成功的 sample 全废。

| 模块 | 改动 |
|---|---|
| [`metrics/judge_core.py`](metrics/judge_core.py) `judge_pointwise._score` | 签名 `Callable[..., float \| None]`；`try parse_pointwise_score / except ValueError: return None` |
| [`metrics/judge_core.py`](metrics/judge_core.py) `g_eval._score` | 签名 `Callable[..., dict[str, float \| None]]`；单次 sample parse 失败 → 跳过；该维 valid 全空 → 维返 None；部分失败按 valid mean |
| [`metrics/judge_rag.py`](metrics/judge_rag.py) `judge_answer_correctness` | `except ValueError: return 0.0` → `return None`；其余 4 处 `return 0.0`（target/pred 空 / TP+FP+FN=0 / P+R=0）保留——是 degenerate-input 路径，0.0 = 合法最低分 |
| [`metrics/judge_rag.py`](metrics/judge_rag.py) `judge_answer_relevancy` | `except ValueError: return 0.0` → `return None`；line 369 "no pred → 0.0" 保留（empty response 的 relevancy=0 是合法最低分） |
| [`api.py`](api.py) `SampleResult.metrics` | 类型 `dict[str, float \| dict[str, float]]` → `dict[str, float \| None \| dict[str, float \| None]]`（widening） |
| [`tasks/base.py`](tasks/base.py) `Task.aggregation` 抽象签名 | 返回类型 `Callable[..., float]` → `Callable[..., float \| None]`；老 task 仍 float（return-type covariance 兼容） |
| [`tasks/qa_open.py`](tasks/qa_open.py) `process_results` + `_judge_pointwise_mean` | judge 返 None 时不写 `metrics["judge_pointwise"]` 键；`_judge_pointwise_mean` 全 None / 空 srs → return None |
| [`tasks/safety.py`](tasks/safety.py) `process_results` | judge 返 None 时不写 `metrics["judge_safety_score"]`（`safety_aggregation_funcs._judge_safety_score` phase 7 wave 2 已 None-skipping，0 改动） |
| [`tasks/agent_traj.py`](tasks/agent_traj.py) `process_results` + `_mean_metric` | g_eval 返 dict 中各维 None 时跳过取 valid mean；全 None → 不写 `plan_quality` 键；`_plan_<dim>` 私有键写 None 保 drill-down 价值；`_mean_metric` 全 None / 空 srs → return None |
| [`tasks/rag_qa.py`](tasks/rag_qa.py) `process_results` + `_mean_metric` | 5 closure 统一 None-check（answer_correctness / answer_relevancy 真有 None 路径，其余 3 个仍只回 float，但访问统一稳健）；`_mean_metric` 同形 |
| 测试增量 +6 | `test_judge_core.py` 3 条（pointwise None / g_eval all-None dim / g_eval partial-fail valid mean）+ `test_judge_rag.py` 2 条（answer_correctness None / answer_relevancy None）+ `test_qa_open_score.py` 1 条（aggregator 全 None → None） |
| 全量回归 | 378 → 384 测试通过；ollama live 实跑 `agent_traj score+judge garbage.jsonl` 不再 crash，`plan_quality=1.0`、`efficiency.judge.*` 真数据全有 |

### Implementation

| 侧面 | 做法 | 哲学 |
|---|---|---|
| `parse_pointwise_score` 不变 | 仍 raise ValueError on no-int input；[`tests/test_judge_core.py`](tests/test_judge_core.py) `test_parse_pointwise_score_invalid_raises` 显式锁的"完全无 int 时抛异常"契约保留 | parser 层是"协议 / 编码层"的鲁棒锁；closure 层是"应用 / 系统层"的鲁棒边界——分层而非合层 |
| 闭包层 try/except | judge_pointwise / g_eval / judge_answer_correctness / judge_answer_relevancy 4 处 ValueError → None | 单条 LM 输出失败不该把全 run 拖死；与 phase 8 wave 3 立的"LM 内禀局限非工程问题、但工程边界要兜住"原则一致 |
| 1-5 scale 0 越界 → None 占位 | judge_pointwise / g_eval / judge_safety_score / judge_answer_relevancy 全部"未测得"路径用 None | 与 phase 7 wave 2 P2 立的"None vs 0 语义分离"原则同形扩展；本次只是把 P2 立的原则从"切片为空"扩展到"解析失败" |
| degenerate-input 0.0 保留 | judge_answer_correctness 的 target/pred 空 / TP+FP+FN=0 / P+R=0；judge_answer_relevancy 的 empty pred；judge_faithfulness / judge_context_precision / judge_context_recall 的"无 contexts / 无 claims" | empty 输入的 F1=0 / relevancy=0 是合法最低分（数学上良定义），与"未测得"语义不同——保留 0 防止 None 蔓延到所有 0 路径 |
| aggregator 类型 widening | `Task.aggregation` 抽象签名返回 `Callable[..., float \| None]`；老 task 全 float 仍兼容（return-type covariance） | minimum-impact API 演进；下游 CLI `_fmt_kv` (`None → <n/a>`) + storage `dataclasses.asdict` (`None → JSON null`) 均 phase 7 wave 2 已就位，0 改动 |
| 行业对齐 | inspect_ai `Score.value=None` + Scorer.error 协议 / OpenAI Evals "graded N/A" / Langfuse failed=True artifact——本项目走 None 占位是中间偏现代的工程选择 | 与 lm-evaluation-harness 旧 0/nan 默认相比是 strict improvement；与 inspect_ai / OpenAI Evals 同代默认对齐；DECISIONS §1 / §7.D 已多处 cite inspect_ai 作 nested 派 / Scorer 派参照 |

### Options considered

**① fallback 语义**：

- **方案 A return 0.0（与 judge_rag pre-fix 一致）**：最小 diff；缺点：1-5 scale 0 越界，pollutes mean；与 phase 7 wave 2 P2 立的 None 占位原则矛盾
- 方案 B return scale midpoint (3.0 for 1-5)：mean 影响最小；缺点：与项目体例不一致需要单独 ADR 解释；"用 3.0 当未测得占位" 仍是"假数据"
- **方案 C return None（选）**：与 phase 7 wave 2 P2 同形扩展；aggregator 自然过滤；CLI `<n/a>` 渲染分支已就位；行业 (inspect_ai / OpenAI Evals) 同代默认；用户已显式确认（plan ask 阶段）

**② judge_rag 5 closure 是否全部对齐 None**：

- **只改 ValueError 路径（选）**：`judge_answer_correctness:244` + `judge_answer_relevancy:375` 两处 parse failure 改 None；其余 4+ 处 0.0 是 degenerate-input（target/pred 空 / TP+FP+FN=0 / P+R=0 / no contexts / no claims / empty pred）保留——0 在这些场景下数学上良定义，"empty input 的最低分"与"未测得"语义不同
- 全部 None propagation：scope creep；degenerate-input 路径的 0 失去合法语义；与 phase 7 P2 立的 0 vs None 语义分离矛盾

**③ supersede 范围**：

- **partial supersede（选）**：phase 4 [`metrics/judge_rag.py`](metrics/judge_rag.py) 那 2 处 ValueError → 0.0 fallback 是 phase 4 写的，**早于** phase 7 wave 2 立的 None 占位原则；本次顺手对齐让两套 fallback 风格收敛——这是 cross-phase 旧债的 principled 清理
- 全 supersede：包括 degenerate-input 路径，违反② 的 scope 决策

**④ aggregator empty srs (`if not srs`) 分支**：

- **return None（选）**：与 None propagation 协议一致；空 srs 等价于"未测得"（无样本可看）；与 safety `_refusal_rate` `None if not srs` 同形
- return 0.0（pre-fix 行为）：保留兼容性，但与 wave 4 None 占位协议不一致；空 srs 在跨 run 索引里被 0 vs 真 0 混淆

### Decision

- **方案 C** None propagation：closure 层 try/except → return None；aggregator 过滤 None → 全空时 return None
- **partial supersede**：仅 judge_rag 两处 ValueError fallback 改 None；degenerate-input 0.0 保留
- **API widening**：`Task.aggregation` 抽象签名 + `SampleResult.metrics` 类型同步 widen，老 task 全兼容
- 测试增量 +6 条覆盖 6 个 None 路径；全量 378 → 384 通过；ollama live 实测 garbage stub 不再 crash

### Risks / 显式让步

| 项 | 内容 |
|---|---|
| `_plan_<dim>` 私有键存 None | drill-down 价值优先：cross-run 比较单维 plan_quality 时同条 doc 在不同 run 间可能 None vs float，消费方需 `if v is not None: ...` 防御。私有键不上聚合面板，影响仅 drill-down 工具 |
| aggregator empty srs 改 None 是行为变更 | 老调用方若依赖 `0.0` 当 default 会拿到 None；目前项目内调用全经 CLI / storage 路径，CLI `_fmt_kv` 已 None-aware，storage `asdict` → JSON `null` 落盘正确；外部调用方需 audit |
| judge_answer_correctness 的 0.0 vs None 区分对消费者透明度依赖 docstring | 同一 metric 同时存在 0.0 (degenerate input) 和 None (parse failure) 两种语义；下游 dashboard 区分这两者要查 sample.metrics 是否含 key + 是否 None，比单一 None 更复杂——但语义更精确。诊断时配合 `sample.prediction` raw text 看 |

### Supersession 链

- 本 ADR partial supersede [`metrics/judge_rag.py`](metrics/judge_rag.py) `judge_answer_correctness:244` + `judge_answer_relevancy:375` 在 phase 4 §4 立的 0.0 fallback；其余 degenerate-input 路径不变
